...
use MyPlot\generator\BasicPlotGenerator;
use MyPlot\generator\EndPlotGenerator;
use MyPlot\generator\NetherPlotGenerator;
...
class MyPlot extends PluginBase {
	...
	public function getLevelSettings(string $worldName): PlotLevelSettings {
		return new PlotLevelSettings($worldName);
	}

	public function getDefaultWorldSettings(): array {
		return array_filter((array) $this->getConfig()->get("DefaultWorld", []), function ($key): bool {
	...
	public function isLevelLoaded(string $worldName): bool {
		return stristr($worldName, "plot") !== false; // all the plot worlds always have plot settings n shit
	}
	 ...
	 * @return bool
	 */
	public function generateLevel(string $worldName): bool {
		$type = match (true) {
			stristr($worldName, "basic") !== false => 0,
			stristr($worldName, "nether") !== false => 1,
			stristr($worldName, "end") !== false => 2,
			default => 0
		};
		$generator = match ($type) {
			0 => BasicPlotGenerator::NAME,
			1 => NetherPlotGenerator::NAME,
			2 => EndPlotGenerator::NAME,
			default => BasicPlotGenerator::NAME
		};
		$settings = $this->getLevelSettings($worldName);
		$ev = new MyPlotGenerationEvent($worldName, $generator, $settings);
		$ev->call();
		$worldManager = $this->getServer()->getWorldManager();
		if ($ev->isCancelled() || $worldManager->isWorldGenerated($worldName)) {
			return false;
		}
		$generator = GeneratorManager::getInstance()->getGenerator($generator)?->getGeneratorClass();
		if (is_null($generator)) $generator = GeneratorManager::getInstance()->getGenerator("normal")->getGeneratorClass();
		$options = WorldCreationOptions::create()->setGeneratorClass($generator)->setGeneratorOptions($worldName);
		$return = $worldManager->generateWorld($worldName, $options);
		$world = $worldManager->getWorldByName($worldName);
		if ($world !== null)
			$world->setSpawnLocation(new Vector3(0.5, $settings->groundHeight + 1, 0.5));
		return $return;
	}
	 ...
	 * @return Plot[]
	 */
	public function getPlotsOfPlayer(int $xuid, string $worldName = ""): array {
		return $this->dataProvider->getPlotsByOwner($xuid, $worldName);
	}
	...
	private function getPlotFast(float &$x, float &$z, PlotLevelSettings $plotLevel): ?Plot {
		...
		if ($x >= 0) {
			...
			$difX = abs((floor($x) - $plotSize) % $totalSize);
			$x = (int)ceil(($x - $plotSize) / $totalSize);
		}
		if ($z >= 0) {
			$difZ = floor($z) % $totalSize;
			$z = (int)floor($z / $totalSize);
		} else {
			$difZ = abs((floor($z) - $plotSize) % $totalSize);
			$z = (int)ceil(($z - $plotSize) / $totalSize);
		}
	...
	public function mergePlots(Plot $plot, int $direction, int $maxBlocksPerTick = 256): bool {
		...
		if ($alreadyMerged === false && $newPlot->isMerged()) {
	 ...
	 * @return bool
	 */
	public function claimPlot(Plot $plot, int $claimer, string $plotName = ""): bool {
		$newPlot = clone $plot;
	...
	public function setPlotBiome(Plot $plot, Biome $biome): bool {
		...
		$plot = $ev->getPlot();
		if (defined(BiomeIds::class . "::" . $plot->biome) && is_int(constant(BiomeIds::class . "::" . $plot->biome))) {
		...
		foreach ($chunks as $id => $chunk) {
			...
			for ($x = 0; $x < 16; ++$x) {
				for ($z = 0; $z < 16; ++$z) {
					...
					if ($chunkPlot instanceof Plot && $chunkPlot->isSame($plot)) {
						$chunk->setBiomeId($x, $plotWorld->groundHeight, $z, $biome->getId());
					}
				}
			}
	...
	public function addPlotHelper(Plot $plot, int $xuid): bool {
		if (!$plot->addHelper($xuid)) {
			return false;
		}
	...
	public function removePlotHelper(Plot $plot, int $xuid): bool {
		$newPlot = clone $plot;
		$ev = new MyPlotSettingEvent($plot, $newPlot);
		if (!$newPlot->removeHelper($xuid))
			$ev->cancel();
		$ev->call();
		if ($ev->isCancelled()) {
			return false;
	...
	public function addPlotDenied(Plot $plot, int $xuid): bool {
		$newPlot = clone $plot;
		$ev = new MyPlotSettingEvent($plot, $newPlot);
		if (!$newPlot->denyPlayer($xuid))
			$ev->cancel();
		$ev->call();
		if ($ev->isCancelled()) {
			return false;
	...
	public function removePlotDenied(Plot $plot, int $xuid): bool {
		$newPlot = clone $plot;
		$ev = new MyPlotSettingEvent($plot, $newPlot);
		if (!$newPlot->unDenyPlayer($xuid))
			$ev->cancel();
		$ev->call();
		if ($ev->isCancelled()) {
			return false;
	...
	/**
	 * Resets the price, adds the money to the player's account && claims a plot in a players name
	 *
	 * @api
	 *
	 * @param Plot $plot
	...
	public function buyPlot(Plot $plot, Player $player): bool {
		...
		foreach ($this->dataProvider->getMergedPlots($plot) as $mergedPlot) {
			...
			$newPlot->owner = (int)$player->getXuid();
			$newPlot->updateUser();
			$newPlot->helpers = [];
			$newPlot->denied = [];
			$newPlot->price = 0.0;
	 ...
	 * @api
	 *
	 * @return array<string, PlotLevelSettings>
	 */
	public function getPlotLevels(): array {
		$worlds = [];
		foreach ($this->getServer()->getWorldManager()->getWorlds() as $world) {
			if (stristr($world->getFolderName(), "plot") !== false) $worlds[$world->getFolderName()] = $this->getLevelSettings($world->getFolderName());
		}
		return $worlds;
	}
	 ...
	 * @return int
	 */
	public function getMaxPlotsOfPlayer(User|PrisonPlayer $player, ?World $world = null): int {
		if (Core::getInstance()->getNetwork()->getThisServer()->getTypeId() == "event" && !(!$player instanceof User && $player->isTier3())) {
			return 1;
		}

		if ($player instanceof User && is_null($world)) return 1;

		$rank = $player->getRank();
		$world = ($world == null ? $player->getWorld()->getDisplayName() : $world->getDisplayName());
		switch ($world) {
			case "plots":
			case "s0plots":
				switch ($rank) {
	...
	private function teleportMiddle(Player $player, Plot $plot): bool {
		...
		$safeMid = $mid->getWorld()->getSafeSpawn($mid);
		return $player->teleport($safeMid);
	}

	/* -------------------------- Non-API part -------------------------- */
	public function onLoad(): void {
		@mkdir($this->getDataFolder() . "worlds", 0777, true);
		$this->getLogger()->debug(TF::BOLD . "Loading...");
		self::setInstance($this);
		$this->getLogger()->debug(TF::BOLD . "Loading Configs");
		$this->reloadConfig();
		$this->getLogger()->debug(TF::BOLD . "Loading Plot Generators");
		GeneratorManager::getInstance()->addGenerator(BasicPlotGenerator::class, "myplot", \Closure::fromCallable(function (string $preset): ?InvalidGeneratorOptionsException {
			if ($preset === "") {
				return null;
			}
			try {
				BasicPlotGenerator::convertSeed($preset);
				return null;
			} catch (InvalidGeneratorOptionsException $e) {
				return $e;
			}
		}), true);
		GeneratorManager::getInstance()->addGenerator(BasicPlotGenerator::class, "plot:basic", \Closure::fromCallable(function (string $preset): ?InvalidGeneratorOptionsException {
			if ($preset === "") {
				return null;
			}
			try {
		...
		GeneratorManager::getInstance()->addGenerator(NetherPlotGenerator::class, "plot:nether", \Closure::fromCallable(function (string $preset): ?InvalidGeneratorOptionsException {
			if ($preset === "") {
				return null;
			}
			try {
				NetherPlotGenerator::convertSeed($preset);
				return null;
			} catch (InvalidGeneratorOptionsException $e) {
				return $e;
			}
		}), true);
		GeneratorManager::getInstance()->addGenerator(EndPlotGenerator::class, "plot:end", \Closure::fromCallable(function (string $preset): ?InvalidGeneratorOptionsException {
			if ($preset === "") {
				return null;
			}
			try {
				EndPlotGenerator::convertSeed($preset);
				return null;
			} catch (InvalidGeneratorOptionsException $e) {
				return $e;
			}
		...
		if ($this->getConfig()->get("FastClearing", false) && $this->getServer()->getPluginManager()->getPlugin("WorldStyler") === null) {
	...
	public function onEnable(): void {
		...
		$this->dataProvider = new MySQLProvider($this, 256);
		...
		$eventListener = $this->eventListener = new EventListener($this);
