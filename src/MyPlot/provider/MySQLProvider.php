...
class MySQLProvider extends DataProvider {
	...
	public function __construct(MyPlot $plugin, int $cacheSize = 0, array $settings = []) {
		...
		$type = Core::getInstance()->getNetwork()->getThisServer()->getTypeId();
		$name = "plots";

		parent::__construct($plugin, $cacheSize);
		$this->settings = $settings;

		$creds = array_merge(file("/[REDACTED]"), ["prison_" . $type]);
		foreach($creds as $key => $cred) $creds[$key] = str_replace("\n", "", $cred);
		$this->db = new \mysqli(...$creds);

		$this->db->query("CREATE TABLE IF NOT EXISTS plots (id INT PRIMARY KEY AUTO_INCREMENT, level TEXT, X INT, Z INT, name TEXT, owner BIGINT(16), helpers TEXT, denied TEXT, biome TEXT, pvp INT, price FLOAT);");
		$this->db->query("CREATE TABLE IF NOT EXISTS mergedPlots (mergedId INT NOT NULL, originId INT NOT NULL);");
		try{
	...
	public function savePlot(Plot $plot) : bool {
		...
		if($plot->id >= 0) {
			$stmt = $this->db->prepare("INSERT INTO plots(id, level, X, Z, name, owner, helpers, denied, biome, pvp, price) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name=VALUES(name), owner=VALUES(owner), helpers=VALUES(helpers), denied=VALUES(denied), biome=VALUES(biome)");
			$pvp = (int) $plot->pvp;
			$stmt->bind_param('isiisssssid', $plot->id, $plot->levelName, $plot->X, $plot->Z, $plot->name, $plot->owner, $helpers, $denied, $plot->biome, $pvp, $plot->price);
	...
	public function deletePlot(Plot $plot) : bool {
		...
		if($plot->isMerged()) {
			if ($plot->id >= 0) {
				...
				$biome = $plot->biome;
				$price = $settings->claimPrice;
				$id = $plot->id;
				$stmt->bind_param('sidi',$biome, $restrictPVP,  $price, $id);
			}else{
				...
				$x = $plot->X;
				$z = $plot->Z;
				$stmt->bind_param('idsii', $restrictPVP, $price,  $levelname, $x, $z);
			}
			$result = $stmt->execute();
			if ($result === false) {
				$this->plugin->getLogger()->error($stmt->error);
	...
	public function getPlot(string $levelName, int $X, int $Z) : Plot {
		...
		if($result !== false and ($val = $result->fetch_array(MYSQLI_ASSOC)) !== null) {
			...
			$pvp = is_numeric($val["pvp"]) ? (bool) $val["pvp"] : false;
			$plot = new Plot($levelName, $X, $Z, (string) $val["name"], (int) $val["owner"], $helpers, $denied, (string) $val["biome"], $pvp, (float) $val["price"], (int) $val["id"]);
		}else{
	 ...
	 * @return Plot[]
	 */
	public function getPlotsByOwner(int $owner, string $levelName = ""): array {
		$owner = $owner;
		$this->reconnect();
		if($levelName === '') {
		...
		while($result !== false and ($val = $result->fetch_array()) !== null) {
			...
			$plots[] = new Plot((string) $val["level"], (int) $val["X"], (int) $val["Z"], (string) $val["name"], (int) $val["owner"], $helpers, $denied, (string) $val["biome"], $pvp, (float) $val["price"], (int) $val["id"]);
		}
	...
	public function getMergeOrigin(Plot $plot) : Plot {
		$stmt = $this->sqlGetMergeOrigin;
	        ...
	        if($result !== false and $val = $result->fetch_array()) {
	                ...
	                $denied = explode(",", (string) $val["denied"]);
	                $pvp = is_numeric($val["pvp"]) ? (bool)$val["pvp"] : null;
	                return new Plot((string) $val["level"], (int) $val["X"], (int) $val["Z"], (string) $val["name"], (int) $val["owner"], $helpers, $denied, (string) $val["biome"], $pvp, (float) $val["price"], (int) $val["id"]);
	        }
	        return $plot;
	}
	...
	private function prepare() : void {
		...
		$stmt = $this->db->prepare("UPDATE plots SET name = '', owner = '', helpers = '', denied = '', biome = ?, pvp = ?, price = ? WHERE level = ? AND X = ? AND Z = ?;");
		if($stmt === false)
			throw new \Exception($this->db->error);
		$this->sqlDisposeMergedPlot = $stmt;
		$stmt = $this->db->prepare("UPDATE plots SET name = '', owner = '', helpers = '', denied = '', biome = ?, pvp = ?, price = ? WHERE id = ?;");
		if($stmt === false)
			throw new \Exception($this->db->error);
